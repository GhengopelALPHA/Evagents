...
const enum {
	...
	CLOCK3,
	EARS,
	xEARS= EARS+NUMEARS-1, //same as xEYES
	BLOOD,
	TEMP,
	PLAYER,
	BOT_SMELL,
...
namespace conf {
	...
	const int NUM_SONGS= 12; // must increment to add new songs
	...
	const int CELL_TICK_RATE= 4; //Tick rate of all cells in the world. IMPACTS PERFORMANCE!!! 1= every cell calculated every tick, 
	 ...
	 // growth/decay rates so no differences should be observed. Setting to 5+ will cause light to look weird, with diminishing performance returns
	...
	//brain settings
	const int BRAINSIZE= 70 + Output::OUTPUT_SIZE + Input::INPUT_SIZE; //(.cfg)
	const float LEARNRATE= 0.001; // CHANGE TO LEARN FROM USER INPUT
	const bool ENABLE_LEARNING= true; //
	const float BRAIN_DIRECTINPUTS= 0.35; //probability of random brain conns on average which will connect directly to inputs
	...
	const float BRAIN_MEMCONNS= 0.02; //probablility of random brain conns which are memory type
	const float BRAIN_MIRRORCONNS= 0.25; //probability of random brain conns on average which will have -w of another on the same box
	const float BRAIN_CONN_ID_MUTATION_STD_DEV= 1.0; //standard dev. for the brain connection ID change mutation.
	...
	const bool KEEP_MIRRORED_EYES= true; //(.cfg) NOT IMPLEMENTED FULLY
	...
	const float DEFAULT_MUTCHANCE= 0.22; //(.cfg)
