...
public:
    ...
    void glCreateMenu();
    int m_id, sm1_id, sm2_id, sm3_id, sm4_id; // main right-click menus
    int win1;
    void gluiCreateMenu();
...
private:
    ...
    int live_selection;                  // what bot category are we currently trying to auto-select? see namespace "Select" in settings.h
    ...
    char buf[100];		// text buffers
    char buf2[10];
    int modcounter;		// graphical tick counter. BUGGY, DO NOT USE FOR SIMULATION-IMPORTANT FEATURES, use world->modcounter instead!
    int currentTime;	// always up to date live time variable
    int lastUpdate;
    int frames;			// fps/tps counter
    int wWidth;			// window width and height, to reduce glutGet calls
    int wHeight;
    ...
    bool uiclicked;	 // was the ui clicked recently? used to disable drag functions if initial click was on UI

    int ui_layerpreset;       // This is a complicated visual preset flag. see processLayerPreset() in GLView.cpp for details
    int ui_lad_visual_mode;   // what rendering mode are we using for the Loaded Agent Display?
    int ui_lad_data_mode;     // what data mode are we using for the Loaded Agent Display?
    bool ui_movetiles;        // are we allowing tiles to be moved?
    std::vector<UIElement> maintiles; // list of interactive tile buttons

    float scale4ksupport; // because reasons, we are tracking the scalemult that allows all agents to show a default size.
    bool isrendering_;    // debug feature. gets set to false every tick. set it = true inside a cull or a render block, debug will tell if the item is rendered
};
